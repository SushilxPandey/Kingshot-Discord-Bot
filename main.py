...
async def on_member_join(member):
    ...
    if not config:
        logging.warning(f"Server config missing for guild {member.guild.id}. Please run /setup.")
        return
    ...
    if verify_channel:
        await verify_channel.send(
            ...
            f"To gain full access, verify your Kingshot account using:\n"
            f"```/verify <ingame_id>```"
        )
...
async def setup(interaction: discord.Interaction, start_role_name: str, verified_role_name: str, owner_role_name: str, verify_channel_name: str, general_channel_name: str, allowed_kingdoms: str):
    save_server_config(
...
async def verify(interaction: discord.Interaction, ingame_id: int):
    config = get_server_config(str(interaction.guild.id))
    if not config:
    ...
    if interaction.channel.name != verify_channel_name:
        await interaction.response.send_message(f"Use #{verify_channel_name} to verify.", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.response.send_message("Kingshot API unreachable. Try later.", ephemeral=True)
        return

    if kingdom not in allowed_kingdoms:
        await interaction.response.send_message(f"Players from Kingdom {kingdom} cannot verify here.", ephemeral=True)
        return

    nick = f"[{kingdom}] {data.get('alliance','')}- {ingame_name}"[:32]
    ...
    save_player(discord_id, ingame_name, ingame_id, kingdom, data.get("alliance",""))
    if verified_role_obj:
        await interaction.user.add_roles(verified_role_obj)
    if start_role_obj:
        await interaction.user.remove_roles(start_role_obj)
    await interaction.user.edit(nick=nick)
    await interaction.response.send_message(f"Verified! Welcome {interaction.user.mention}.")
...
async def unverify(interaction: discord.Interaction, member: discord.Member):
    ...
    verified_role_obj = discord.utils.get(interaction.guild.roles, name=config[2])
    start_role_obj = discord.utils.get(interaction.guild.roles, name=config[1])
    if verified_role_obj and verified_role_obj in member.roles:
        await member.remove_roles(verified_role_obj)
    if start_role_obj:
        await member.add_roles(start_role_obj)
...
async def mystats(interaction: discord.Interaction):
    ...
    if not player:
        await interaction.response.send_message("Verify first using /verify.", ephemeral=True)
        return
    info = await get_player_info(player[2])
    data = info.get("data", {})
...
@bot.tree.command(name="age", description="Show kingdom age.")
async def age(interaction: discord.Interaction, kingdom_id: int):
    try:
        stats = await get_kingdom_stats(kingdom_id)
        open_time_str = stats["data"]["servers"][0]["openTime"]
        open_time = datetime.fromisoformat(open_time_str.replace("Z","+00:00"))
        now = datetime.now(timezone.utc)
        days = (now - open_time).days
        embed = discord.Embed(title=f"Kingdom {kingdom_id} Age", description=f"{days} days open.", color=discord.Color.green())
    ...
    for guild in bot.guilds:
        ...
        for member in verified_role.members:
            try:
                ...
                logging.error(f"Auto-sync error {guild.id} {member.id}: {e}")
