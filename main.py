...
async def setup(interaction: discord.Interaction, start_role_name: str, verified_role_name: str, owner_role_name: str, verify_channel_name: str, general_channel_name: str, allowed_kingdoms: str):
    ...
    # allowed_kingdoms is a comma-separated string like "466,467"
    kingdoms = [int(k.strip()) for k in allowed_kingdoms.split(",") if k.strip().isdigit()]
    save_server_config(
...
async def hello(interaction: discord.Interaction):
    ...
    if interaction.channel.name != general_channel:
        await interaction.response.send_message(f"Please use this command in the #{general_channel} channel only.", ephemeral=True)
        return
...
async def verify(interaction: discord.Interaction, ingame_id: int, alliance: str):
    config = get_server_config(str(interaction.guild.id))
    if not config:
        await interaction.response.send_message("Server configuration not found. Please ask an admin to run /setup.", ephemeral=True)
        return
    ...
    if interaction.channel.name != verify_channel_name:
        await interaction.response.send_message(f"Please use this command in the #{verify_channel_name} channel only.", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.response.send_message("Could not reach the Kingshot API. Try again later.", ephemeral=True)
        return

    kingdom = player_info["data"]["kingdom"]
    ingame_name = player_info["data"]["name"]

    if kingdom not in allowed_kingdoms:
        await interaction.response.send_message(f"Your kingdom {kingdom} is not allowed on this server.", ephemeral=True)
        return
    ...
    if existing:
        ...
        await interaction.response.send_message("You are already verified! Contact an admin to update your info.", ephemeral=True)
        return
...
async def mystats(interaction: discord.Interaction):
    ...
    if not player:
        await interaction.response.send_message("You are not verified yet. Please verify your account first using /verify.", ephemeral=True)
        return

    player_info = await get_player_info(player[2])
    if not player_info or "data" not in player_info:
        await interaction.response.send_message("Could not retrieve your stats from the Kingshot API.", ephemeral=True)
        return
...
@bot.tree.command(name="age", description="Tracks the age of the Kingshot server")
async def age(interaction: discord.Interaction, kingdom_id: int):
    try:
        ...
        await interaction.response.send_message(f"Kingdom {kingdom_id} was not found.", ephemeral=True)
